...
export async function exportExcelReport<T extends Record<string, unknown>>(options: ExcelExportOptions<T>) {
  ...
  for (const sheetConfig of options.sheets) {
    ...
    sheet.getCell("A1").value = options.reportTitle;
    sheet.getCell("A1").font = { size: 16, bold: true };
    sheet.getCell("A2").value = `Tenant: ${options.tenantName}`;
    sheet.getCell("A3").value = `Branch: ${options.branchName}`;
    sheet.getCell("A4").value = `Date Range: ${formatDateRange(options.dateFrom, options.dateTo)}`;
    sheet.getCell("A5").value = `Filters: ${options.filtersSummary ?? "N/A"}`;
    ...
    headerRow.values = sheetConfig.columns.map((column) => column.header);
