...
import { getFileAccessLogs, type AccessLogDto } from "@/api/auditApi";
...
export function FileAccessLogPage() {
  const language = useLanguage();
  const [filters, setFilters] = useState<AuditFilterState>(defaultAuditFilters);
  const query = useQuery({ queryKey: ["audit-file-logs", filters], queryFn: () => getFileAccessLogs(stripSearch(filters)) });
