...
type Row = { date: string; voucherNumber: string; voucherType: string; particulars: string; narration: string; debit: number; credit: number; balance: number; baseDebit: number; baseCredit: number; baseBalance: number };
...
export function LedgerReportPage() {
  return <AccountingReportPage<Row> title={lt("Ledger Report")} reportType="ledger" needsAccount mapRows={(data) => Array.isArray(data) ? data as Row[] : []} columns={columns} summaryBuilder={(rows) => [{ label: lt("Entries"), value: rows.length }]} totalsBuilder={(rows) => [{ label: lt("Total Debit"), value: rows.reduce((s, x) => s + x.debit, 0).toFixed(2) }, { label: lt("Total Credit"), value: rows.reduce((s, x) => s + x.credit, 0).toFixed(2) }]} />;
}
