...
export function TrialBalancePage() {
  return <AccountingReportPage<Row> title={lt("Trial Balance")} reportType="trial-balance" mapRows={(data) => Array.isArray(data) ? data as Row[] : []} columns={columns} showCurrency totalsBuilder={(rows) => [{ label: lt("Opening Debit"), value: rows.reduce((s, x) => s + x.openingDebit, 0).toFixed(2) }, { label: lt("Opening Credit"), value: rows.reduce((s, x) => s + x.openingCredit, 0).toFixed(2) }, { label: lt("Closing Debit"), value: rows.reduce((s, x) => s + x.closingDebit, 0).toFixed(2) }, { label: lt("Closing Credit"), value: rows.reduce((s, x) => s + x.closingCredit, 0).toFixed(2) }]} />;
}
