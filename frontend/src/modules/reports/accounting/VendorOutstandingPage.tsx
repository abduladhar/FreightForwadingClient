...
const columns: ColumnDef<Row>[] = [{ accessorKey: "partyName", header: lt("Vendor") }, { accessorKey: "documentNumber", header: lt("Document") }, { accessorKey: "documentDate", header: lt("Date") }, { accessorKey: "dueDate", header: lt("Due") }, { accessorKey: "totalAmount", header: lt("Total") }, { accessorKey: "paidAmount", header: lt("Paid") }, { accessorKey: "outstandingAmount", header: lt("Outstanding") }, { accessorKey: "baseOutstandingAmount", header: lt("Base Outstanding") }];
export function VendorOutstandingPage() {
  return <AccountingReportPage<Row> title={lt("Vendor Outstanding")} reportType="vendor-outstanding" needsVendor mapRows={(data) => Array.isArray(data) ? data as Row[] : []} columns={columns} totalsBuilder={(rows) => [{ label: lt("Outstanding"), value: rows.reduce((s, x) => s + x.outstandingAmount, 0).toFixed(2) }]} />;
}
