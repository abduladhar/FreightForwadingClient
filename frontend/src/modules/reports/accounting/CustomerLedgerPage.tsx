...
export function CustomerLedgerPage() {
  return <AccountingReportPage<Row> title={lt("Customer Ledger")} reportType="customer-ledger" needsCustomer mapRows={(data) => Array.isArray(data) ? data as Row[] : []} columns={columns} showCurrency totalsBuilder={(rows) => [{ label: lt("Total Debit"), value: rows.reduce((s, x) => s + x.debit, 0).toFixed(2) }, { label: lt("Total Credit"), value: rows.reduce((s, x) => s + x.credit, 0).toFixed(2) }]} />;
}
