...
export function CustomerOutstandingPage() {
  ...
  const currencies = useQuery({ queryKey: ["customer-outstanding", "currencies"], queryFn: getCurrencies });
  const currencyById = useMemo(() => new Map((currencies.data ?? []).map((currency) => [currency.id, currency.currencyCode])), [currencies.data]);
  const formatNumber = useMemo(
    () => new Intl.NumberFormat(workspace.cultureCode, { maximumFractionDigits: 0 }),
    [workspace.cultureCode]
  );
  ...
  const columns = useMemo<ColumnDef<Row>[]>(() => [
    { accessorKey: "partyName", header: lt("Customer") },
    ...
    <AccountingReportPage<Row>
      ...
      totalsBuilder={(rows) => {
        ...
        rows.forEach((row) => {
          ...
          outstandingByCurrency.set(code, (outstandingByCurrency.get(code) ?? 0) + row.outstandingAmount);
        });
        const originalCurrencyTotals = Array.from(outstandingByCurrency.entries()).map(([code, amount]) => ({
      ...
      prepareClientExport={(rows) => {
        ...
        const exportColumns = [
          { key: "partyName", label: lt("Customer") },
        ...
        Array.from(outstandingByCurrency.entries()).forEach(([code, amount]) => {
          exportRows.push({
