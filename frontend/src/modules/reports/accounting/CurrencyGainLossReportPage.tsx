...
export function CurrencyGainLossReportPage() {
  return <AccountingReportPage<Row> title={lt("Currency Gain/Loss")} reportType="currency-gain-loss" mapRows={(data) => Array.isArray(data) ? data as Row[] : []} columns={columns} showCurrency totalsBuilder={(rows) => [{ label: lt("Gain Total"), value: rows.reduce((s, x) => s + x.gainAmount, 0).toFixed(2) }, { label: lt("Loss Total"), value: rows.reduce((s, x) => s + x.lossAmount, 0).toFixed(2) }]} />;
}
