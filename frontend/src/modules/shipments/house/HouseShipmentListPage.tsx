...
import { Eye, FilePlus2, FileText, MoreHorizontal, Pencil, Plus, Printer, Receipt, SlidersHorizontal, TrendingUp } from "lucide-react";
import { searchHouseShipments, type HouseShipmentDto } from "@/api/houseShipmentApi";
...
export function HouseShipmentListPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={lt("House Shipments")} description={lt("House shipment planning, loading, documents, labels, and profit preview.")} actions={<><AuditTrailButton /><PermissionButton asChild permission="HouseShipment.Create"><Link to="/house-shipments/new"><Plus className="h-4 w-4" />{lt("New House Shipment")}</Link></PermissionButton></>} />
      <Card>
        <CardContent className="space-y-4 pt-6">
          ...
          <DataTable
            ...
            rowActions={(row) => <HouseShipmentActions row={row} hasPermission={hasPermission} />}
          />
        </CardContent>
      </Card>
    </div>
...
function HouseShipmentActions({ row, hasPermission }: { row: HouseShipmentDto; hasPermission: (permission?: string | string[]) => boolean }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-56">
        {hasPermission("HouseShipment.Read") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}`}><Eye className="mr-2 h-4 w-4" />{lt("View Shipment")}</Link></DropdownMenuItem> : null}
        {hasPermission("HouseShipment.Update") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/edit`}><Pencil className="mr-2 h-4 w-4" />{lt("Edit Shipment")}</Link></DropdownMenuItem> : null}
        {hasPermission("HouseShipment.Update") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/status`}><FileText className="mr-2 h-4 w-4" />{lt("Update Status")}</Link></DropdownMenuItem> : null}
        {hasPermission("HouseShipment.Print") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/label`} target="_blank" rel="noreferrer"><Printer className="mr-2 h-4 w-4" />{lt("Print Labels")}</Link></DropdownMenuItem> : null}
        {hasPermission("HouseShipment.Print") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/note`} target="_blank" rel="noreferrer"><FileText className="mr-2 h-4 w-4" />{lt("Print Job Card")}</Link></DropdownMenuItem> : null}
        {hasPermission("Invoice.Read") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/invoices`}><Receipt className="mr-2 h-4 w-4" />{lt("Show Invoices")}</Link></DropdownMenuItem> : null}
        {hasPermission("Invoice.Create") ? <DropdownMenuItem asChild><Link to={`/invoices/new?sourceType=HouseShipment&sourceId=${row.id}&customerId=${row.customerId}`}><Plus className="mr-2 h-4 w-4" />{lt("Create Invoice")}</Link></DropdownMenuItem> : null}
        {hasPermission("VendorBill.Read") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/bills`}><FilePlus2 className="mr-2 h-4 w-4" />{lt("Show Bills")}</Link></DropdownMenuItem> : null}
        {hasPermission("VendorBill.Create") ? <DropdownMenuItem asChild><Link to={`/vendor-bills/new?sourceType=HouseShipment&sourceId=${row.id}&expectedCostAmount=${row.costAmount ?? 0}`}><FilePlus2 className="mr-2 h-4 w-4" />{lt("Create Bill")}</Link></DropdownMenuItem> : null}
        {hasPermission("HouseShipment.Read") ? <DropdownMenuItem asChild><Link to={`/house-shipments/${row.id}/profit`}><TrendingUp className="mr-2 h-4 w-4" />{lt("Profit Preview")}</Link></DropdownMenuItem> : null}
      </DropdownMenuContent>
    </DropdownMenu>
  );
}

function FinanceStatusDropdown({ row }: { row: HouseShipmentDto }) {
