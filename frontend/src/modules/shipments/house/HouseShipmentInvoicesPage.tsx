...
export function HouseShipmentInvoicesPage() {
  ...
  const shipmentQuery = useQuery({
    queryKey: ["house-shipment-invoices-context", shipmentId],
    queryFn: () => getHouseShipment(shipmentId!),
    enabled: Boolean(shipmentId)
  });
  const customerQuery = useQuery({
    queryKey: ["house-shipment-invoices-customer", shipmentQuery.data?.customerId],
    queryFn: () => getCustomer(shipmentQuery.data!.customerId),
    enabled: Boolean(shipmentQuery.data?.customerId)
  });
  const invoicesQuery = useQuery({
    queryKey: ["house-shipment-invoices", shipmentId, pageNumber, pageSize, search],
    queryFn: () => searchInvoices({ pageNumber, pageSize, search, sourceType: "HouseShipment", sourceId: shipmentId! }),
    enabled: Boolean(shipmentId)
  });
  const currenciesQuery = useQuery({ queryKey: ["tenant-currencies", "house-shipment-invoices"], queryFn: getTenantCurrencies });
  ...
  const createInvoicePath = `/invoices/new?sourceType=HouseShipment&sourceId=${encodeURIComponent(shipmentId)}&customerId=${encodeURIComponent(shipment.customerId)}`;

  return (
    <div className="space-y-4">
      <PageHeader
        title={lt("House Shipment Invoices")}
        description={`Invoices for ${shipment.houseShipmentNumber}`}
        actions={
          <>
            <AuditTrailButton />
      ...
      <ShipmentContext shipment={shipment} customerName={customerName} />
      ...
      <Card>
        <CardContent className="pt-6">
          <DataTable
            ...
            search={search}
            searchPlaceholder={lt("Search invoice number, charge, or status")}
...
function ShipmentContext({ shipment, customerName }: { shipment: HouseShipmentDto; customerName: string }) {
