...
import { getHouseShipment, type HouseShipmentDto } from "@/api/houseShipmentApi";
...
export function HouseShipmentVendorBillsPage() {
  ...
  const shipmentQuery = useQuery({
    queryKey: ["house-shipment-bills-context", shipmentId],
    queryFn: () => getHouseShipment(shipmentId!),
    enabled: Boolean(shipmentId)
  });
  const customerQuery = useQuery({
    queryKey: ["house-shipment-bills-customer", shipmentQuery.data?.customerId],
    queryFn: () => getCustomer(shipmentQuery.data!.customerId),
    enabled: Boolean(shipmentQuery.data?.customerId)
  });
  const billsQuery = useQuery({
    queryKey: ["house-shipment-vendor-bills", shipmentId, pageNumber, pageSize, search],
    queryFn: () => searchVendorBills({ pageNumber, pageSize, search, sourceType: "HouseShipment", sourceId: shipmentId! }),
    enabled: Boolean(shipmentId)
  });
  const currenciesQuery = useQuery({ queryKey: ["tenant-currencies", "house-shipment-vendor-bills"], queryFn: getTenantCurrencies });
  ...
  const approve = useMutation({
    ...
    onSuccess: async () => queryClient.invalidateQueries({ queryKey: ["house-shipment-vendor-bills"] })
  });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={lt("House Shipment Vendor Bills")}
        description={`Expense bills for ${shipment.hawbNumber || shipment.houseShipmentNumber}`}
        actions={
          <>
            <AuditTrailButton />
            <PermissionButton asChild permission="HouseShipment.Read" variant="outline">
              <Link to={`/house-shipments/${shipmentId}`}>
                <Eye className="h-4 w-4" />{lt("House Shipment")}</Link>
            </PermissionButton>
      ...
      <HouseShipmentBillContext shipment={shipment} customerName={customerName} />
      ...
      <Card>
        <CardContent className="pt-6">
          <DataTable
            ...
            search={search}
            searchPlaceholder={lt("Search vendor bill number, HAWB, house shipment number, or status")}
              ...
              <div className="flex flex-wrap justify-end gap-1">
                ...
                {hasPermission("VendorBill.Cancel") && row.status !== "Cancelled" ? (
                  <ConfirmDialog
                    ...
                    variant="danger"
                    onConfirm={async () => {
                      await cancel.mutateAsync({ id: row.id, reason: "Cancelled from house shipment bills page" });
                    }}
                  >
...
function HouseShipmentBillContext({ shipment, customerName }: { shipment: HouseShipmentDto; customerName: string }) {
  const totals = shipment.items.reduce(
    (sum, item) => ({
      pieces: sum.pieces + Number(item.loadedPieces || item.receivedPieces || 0),
      weight: sum.weight + Number(item.loadedWeight || item.receivedWeight || 0),
      volume: sum.volume + Number(item.loadedVolume || item.volumeCbm || 0)
    }),
    ...
    <Card>
      <CardContent className="space-y-4 pt-6">
        <div className="flex flex-wrap items-center gap-2">
          <Badge tone="blue">HAWB {shipment.hawbNumber || "-"}</Badge>
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <Field label={lt("House Shipment No")}>{shipment.houseShipmentNumber}</Field>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-md border p-3">
            ...
            <Field label={lt("Contact")}>{shipment.shipperContactNo || "-"}</Field>
          ...
          <div className="rounded-md border p-3">
            ...
            <Field label={lt("Contact")}>{shipment.consigneeContactNo || "-"}</Field>
...
function formatPort(code?: string | null, name?: string | null, country?: string | null, fallback?: string | null) {
  ...
  const withCountry = [main || fallback, country].filter(Boolean).join(" - ");
  return withCountry || "-";
}
