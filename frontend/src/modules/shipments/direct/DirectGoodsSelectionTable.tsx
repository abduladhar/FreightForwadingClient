...
import type { DirectShipmentItemRequest } from "@/api/directShipmentApi";
...
export function DirectGoodsSelectionTable({
  initialCustomerId,
  onClose,
  onApply
}: {
  ...
  onClose: () => void;
  onApply: (items: DirectShipmentItemRequest[]) => Promise<void> | void;
}) {
  ...
  const [selected, setSelected] = useState<Record<string, DirectShipmentItemRequest>>({});
  const [isApplying, setIsApplying] = useState(false);

  const hasAnyFilter = Boolean(filters.customerId.trim() || filters.grnNumber.trim() || filters.dateFrom.trim() || filters.dateTo.trim());

  const availableQuery = useQuery({
    queryKey: ["direct-shipment-grn-filter-available", filters.customerId],
    queryFn: () => getAvailableGoods(filters.customerId || undefined),
    enabled: hasAnyFilter
  });

  const receiptQuery = useQuery({
    queryKey: ["direct-shipment-grn-filter-receipts", filters.customerId, filters.grnNumber],
    queryFn: () =>
      searchGoodsReceipts({
        pageNumber: 1,
        pageSize: 500,
  ...
  const rows = useMemo<EnrichedAvailableRow[]>(() => {
    ...
    const list = available
      .map((item) => {
        if ((item.availablePieces ?? 0) <= 0) return null;
    ...
    return list.filter((x) => {
      ...
      if (filters.dateFrom) {
        ...
        if (new Date(x.goodsReceiptDateTime) < from) return false;
      }
      if (filters.dateTo) {
        const to = new Date(`${filters.dateTo}T23:59:59.999`);
        if (new Date(x.goodsReceiptDateTime) > to) return false;
      }
      return true;
    });
  }, [availableQuery.data, filters.dateFrom, filters.dateTo, filters.grnNumber, hasAnyFilter, receiptQuery.data?.items]);

  const selectedRows = useMemo(() => Object.values(selected).filter((x) => x.pieces > 0), [selected]);
      ...
      <div className="max-h-[90vh] w-full max-w-7xl overflow-hidden rounded-lg border bg-white shadow-2xl">
        ...
        <div className="space-y-4 p-4">
          <div className="grid gap-3 md:grid-cols-5">
            ...
            />
            <Input placeholder={lt("Filter: Goods Receipt Note Number")} value={filters.grnNumber} onChange={(e) => setFilters((prev) => ({ ...prev, grnNumber: e.target.value }))} />
            <Input type="date" value={filters.dateFrom} onChange={(e) => setFilters((prev) => ({ ...prev, dateFrom: e.target.value }))} />
            <Input type="date" value={filters.dateTo} onChange={(e) => setFilters((prev) => ({ ...prev, dateTo: e.target.value }))} />
            <Button type="button" variant="outline" onClick={() => setFilters({ customerId: initialCustomerId ?? "", grnNumber: "", dateFrom: "", dateTo: "" })}>{lt("Reset Filters")}</Button>
          </div>
              ...
              <table className="min-w-[1200px] w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="p-2 text-right">{lt("Width (Per Item)")}</th>
                  ...
                  {rows.map((item) => {
                    ...
                    return (
                      <tr key={item.goodsReceiptItemId} className="border-t">
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.checked) {
                                next[item.goodsReceiptItemId] = {
                                  ...row,
                                  pieces: row.pieces || item.availablePieces,
                                  weight: row.weight || item.availableWeight,
                                  volume: row.volume || item.availableVolume,
                        ...
                        <td className="p-2">
                          <Input type="number" min={0} value={row.weight} disabled />
                        </td>
                        <td className="p-2">
                          <Input type="number" min={0} value={row.volume} disabled />
                        </td>
                      </tr>
                    );
                  })}
          ...
          <div className="flex items-center justify-between border-t pt-3">
            <p className="text-sm text-muted-foreground">{selectedRows.length} item(s) selected.</p>
