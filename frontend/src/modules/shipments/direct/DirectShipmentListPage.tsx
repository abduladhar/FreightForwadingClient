...
export function DirectShipmentListPage() {
  ...
  const query = useQuery({
    queryKey: ["direct-shipments", pageNumber, pageSize, search, customer, houseWaybillNo, consigneeName, shipperName, originPort, destinationPort, consigneePhoneNo, shipperPhoneNo, invoiceDefined, billDefined, unpaidInvoice, unpaidBill, invoiceStatus, billStatus],
          ...
          {showFilters ? (
            <div className="rounded-lg border bg-slate-50/60 p-3">
              <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-4">
                <FilterField label={lt("Waybill No")}><Input value={houseWaybillNo} placeholder={lt("Master/house waybill")} onChange={(event) => { setHouseWaybillNo(event.target.value); setPageNumber(1); }} /></FilterField>
