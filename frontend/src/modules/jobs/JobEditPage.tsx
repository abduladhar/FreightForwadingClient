...
export function JobEditPage() {
  ...
  const documents = useQuery({ queryKey: ["documents", "Job", id], queryFn: () => getShipmentDocuments("Job", id!), enabled: Boolean(id) });
  ...
  return <div className="space-y-4"><PageHeader title={lt("Edit Job")} description={lt("Update job type, description, and status.")} /><Card><CardContent className="pt-6">{initialValue ? <JobForm initialValue={initialValue} jobNumber={query.data?.jobNumber} isSubmitting={mutation.isPending} onSubmit={async (value) => { await mutation.mutateAsync(value); }} /> : <p className="text-sm text-muted-foreground">{lt("Loading job...")}</p>}</CardContent></Card>{id ? <DocumentUploadPanel moduleName="Job" entityId={id} referenceNumber={query.data?.jobNumber ?? ""} defaultDocumentName={lt("Job Document")} defaultDocumentCategory={lt("Job")} emptyText={lt("No documents uploaded for this job.")} documents={documents.data ?? []} onRefresh={() => void documents.refetch()} /> : null}</div>;
}
